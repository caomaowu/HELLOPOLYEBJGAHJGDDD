...
const SellOrdersTab: React.FC<SellOrdersTabProps> = ({ copyTradingId, active = false }) => {
  ...
  const fetchOrders = useCallback(async (marketTitleOverride?: string) => {
    if (!copyTradingId) return

    const marketTitle = marketTitleOverride ?? marketTitleRef.current
  ...
  }, [copyTradingId, filters.marketId, filters.status, groupByMarket, limit, page])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetchOrders(filters.marketTitle)
    }, 500) // 0.5秒延迟

    return () => clearTimeout(timer)
  }, [active, copyTradingId, fetchOrders, filters.marketTitle])
