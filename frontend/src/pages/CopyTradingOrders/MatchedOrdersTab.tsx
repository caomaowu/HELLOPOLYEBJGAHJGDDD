...
const MatchedOrdersTab: React.FC<MatchedOrdersTabProps> = ({ copyTradingId, active = false }) => {
  ...
  const marketTitleRef = useRef<string | undefined>(filters.marketTitle)
  const buyOrderIdRef = useRef<string | undefined>(filters.buyOrderId)
  const sellOrderIdRef = useRef<string | undefined>(filters.sellOrderId)
  ...
  useEffect(() => {
    marketTitleRef.current = filters.marketTitle
    buyOrderIdRef.current = filters.buyOrderId
    sellOrderIdRef.current = filters.sellOrderId
  ...
  const fetchOrders = useCallback(async (nextFilters?: {
    ...
    const marketTitle = nextFilters?.marketTitle ?? marketTitleRef.current
    const buyOrderId = nextFilters?.buyOrderId ?? buyOrderIdRef.current
    const sellOrderId = nextFilters?.sellOrderId ?? sellOrderIdRef.current
  ...
  }, [copyTradingId, limit, page])

  useEffect(() => {
    if (copyTradingId && active) {
      fetchOrders()
    }
  }, [active, copyTradingId, fetchOrders])
  ...
  }, [active, copyTradingId, fetchOrders, filters.marketTitle, filters.buyOrderId, filters.sellOrderId])
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        ...
        <Button type="primary" onClick={() => void fetchOrders()} icon={<ReloadOutlined />}>{t('common.refresh') || '刷新'}</Button>
      </div>
